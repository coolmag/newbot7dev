...
class YouTubeDownloader:
    ...
    def _get_opts(self, is_search: bool = False) -> Dict[str, Any]:
        """
        Настройки для скорости и надежности.
        """
        opts: Dict[str, Any] = {
            "quiet": True,
            "no_warnings": True,
            ...
            # Фильтр на уровне движка (дублируем защиту)
    ...
    def _find_downloaded_file(self, video_id: str) -> Optional[str]:
        ...
        # Приоритет: MP3 > M4A > Остальное
        valid_files.sort(key=lambda x: 0 if x.endswith(".mp3") else 1 if x.endswith(".m4a") else 2)
    ...
    async def search(
        ...
        **kwargs,
        ...
        """
        Быстрый поиск с фильтрацией мусора.
        """
        ...
        try:
            ...
            # Стоп-слова (фильтруем 10-часовые версии)
            ...
            for e in entries:
                ...
                if any(b in title for b in BANNED):
                    if "mix" not in query.lower(): # Если юзер сам не просил микс
                        continue

                duration = int(e.get("duration") or 0)
                # Если длительность > 15 мин или неизвестна (0) - пропускаем
        ...
        for attempt in range(self._settings.MAX_RETRIES):
            try:
                ...
                if result.error and ("too long" in str(result.error) or "large" in str(result.error)):
                    return result # Не ретраим фатальные ошибки
    ...
    async def download(self, query_or_id: str) -> DownloadResult:
        ...
        try:
            ...
            try:
                ...
                if "too long" in str(e) or "larger" in str(e):
                    return DownloadResult(success=False, error="Файл слишком большой.")
                logger.warning(f"yt-dlp warning: {e}")

            # 3. ПОИСК ФАЙЛА НА ДИСКЕ
            final_path = self._find_downloaded_file(video_id)
            
            if not final_path:
                return DownloadResult(success=False, error="Файл скачался, но не найден.")

            # 4. Метаданные (легкий запрос)
            try:
